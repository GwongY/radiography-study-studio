...
import { buildLayer, layoutHash, localBounds, similarityOf } from './build-physiology-paths.mjs';
import { PATH_ATTRIBUTES } from '../outputs/physiology-path.js';
...
for (const [, { definitions }] of built) {
  for (const definition of definitions) {
    ...
    for (const { ref, page, quote } of definition.sourceRefs) {
      ...
      const text = source.pages?.[String(page)];
...
/* --- the committed payload is the one this code produces ---------------- */
{
  ...
  for (const [layer, { result }] of built) {
    ...
    ok(JSON.stringify(payload.attributes) === JSON.stringify(PATH_ATTRIBUTES), `${layer}: the payload declares the attribute layout the kernel defines`);
    ...
    const resident = payload.routes.reduce((sum, r) => sum + r.vertices * 10 * 4, 0);
...
console.log(`PASS: ${checks} model-side assertions (curated names resolve, quotes are on their pages, refusals are named, landmarks fall in the taught order, the committed payload matches a fresh derivation, budgets hold)`);
