async function physiologyPapillaryVisualCheck(){
 const s=__osteo.state,T=s.THREE;
 const names=['Left','Right'];
 const sheet=document.createElement('canvas');sheet.width=1200;sheet.height=460;
 const ctx=sheet.getContext('2d');ctx.fillStyle='#eef1f4';ctx.fillRect(0,0,1200,460);
 ...
 for(let row=0;row<names.length;row++){
  const originals=s.extraModels.circulatory.meshes.filter(m=>new RegExp('^(?:'+names[row]+'_ventricle(?:_\\d+)?|.*papillary.*'+names[row]+'_ventricle)$','i').test(m.name));
  const scene=new T.Scene(),group=new T.Group(),probe={value:0};let amplitude=0;
  for(const original of originals){
   const mesh=new T.Mesh(original.geometry,original.material.clone());original.updateWorldMatrix(true,false);
   mesh.matrixAutoUpdate=false;mesh.matrix.copy(original.matrixWorld);mesh.visible=true;
   mesh.material.side=T.FrontSide;const papillary=/papillary/i.test(original.name);mesh.material.transparent=!papillary;mesh.material.opacity=papillary?1:.25;mesh.material.depthWrite=papillary;if(papillary)mesh.material.color.setHex(0xffb52e);
   mesh.material.onBeforeCompile=shader=>{original.material.onBeforeCompile(shader);shader.uniforms.uDeform=probe;shader.uniforms.uOn={value:0};};
   mesh.material.customProgramCacheKey=()=>original.material.customProgramCacheKey();group.add(mesh);
  }
  scene.add(group,new T.AmbientLight(0xffffff,1.5));
  const light=new T.DirectionalLight(0xffffff,3);light.position.set(3,6,5);scene.add(light);
  scene.updateMatrixWorld(true);const box=new T.Box3().setFromObject(group),centre=box.getCenter(new T.Vector3()),extent=box.getSize(new T.Vector3()).length()*.6;
 ...
 renderer.dispose();sheet.id='papillaryVisualEvidence';sheet.style.cssText='position:fixed;inset:0;z-index:99999;width:1200px;height:460px';document.body.append(sheet);
