...
async () => {
  ...
  exploreButton.click();
  assert(getComputedStyle(document.getElementById('taskCard')).display==='none','Explore can collapse from the bottom bar');
  exploreButton.click();
  assert(getComputedStyle(document.getElementById('taskCard')).display!=='none','Explore can expand from the bottom bar');
  ...
  const group=s.toolGroup||(s.toolGroup=new s.THREE.Group());
  if(!group.parent)s.scene.add(group);
  group.add(label);hud.updateHudSprites();
  assert(Math.abs(label.scale.x/label.scale.y-label.userData.hud.aspect)<1e-6,'Annotation tags preserve their text aspect ratio');
  assert(label.material.map.image.height>100,'Long annotation text wraps');
  group.remove(label);label.material.map.dispose();label.material.dispose();
