async function physiologyBreathingVisualCheck(){
 const s=__osteo.state,T=s.THREE;
 const names=['Breathing opaque','Breathing translucent'];
 ...
 for(let row=0;row<names.length;row++){
  const originals=[...s.extraModels.muscle.meshes.filter(m=>m.userData.flowClass==='diaphragm'),...s.extraModels.organs.meshes.filter(m=>m.userData.flowClass==='airway'&&/lobe.*lung/i.test(m.name))];
  ...
  for(const original of originals){
   ...
   mesh.material.side=T.FrontSide;mesh.material.transparent=row===1;mesh.material.opacity=row===1?.4:1;mesh.material.depthWrite=row!==1;
 ...
 renderer.dispose();sheet.id='breathingVisualEvidence';sheet.style.cssText='position:fixed;inset:0;z-index:99999;width:1200px;height:460px';document.body.append(sheet);
