...
| `figures.js` | 1874 | figures.js — the diagrams that replaced the hand-drawn ones. |  |
...
| `schedule.js` | 915 | schedule.js — the semester itself: what the syllabus says, and when each |  |
...
|  | 369–811 | Which lessons cover which week |
|  | 812–860 | Where a session sits in time |
|  | 861–915 | Formatting — kept here so the view has no date arithmetic in it |
...
| `depth-picking.js` | 411 | Depth picking |
...
| `live-physiology.js` | 1271 | Live physiology |
...
| `spatial-concept-overlays.js` | 484 | Spatial concept overlays -- cavities, regions, quadrants, planes. |
| `tools-and-capture.js` | 854 | Tools — section cuts, surface ink, pinned labels, capture. |
...
| `navigation-five-destinations.js` | 128 | Navigation -- five destinations, rendered into both the icon rail and |
...
| `coverage.js` | 239 | Coverage report — what the supplied sources actually cover, and what they |
...
| `expansion-items.js` | 1629 | Expansion batch — fills gaps found in a coverage audit. |
| `hss-joints.js` | 1318 | HSS2011 Human Anatomy — joints and the muscles that move them. |
| `hss-modules.js` | 5221 | HSS2011 Human Anatomy — the per-module study items. |
| `hss-osteology.js` | 2060 | HSS2011 Human Anatomy — osteology: bone classification, the axial and |
...
| `hti-items.js` | 872 | HTI17103, drawn from the HTI17101 Exploring Radiography set — the study |
...
| `physiology-depth.js` | 1949 | ABCT2326 depth lessons. These split the mechanism-heavy respiratory and |
| `physiology-items.js` | 6528 | ABCT2326 Human Physiology — the study items. |
...
| `source-lesson-map.js` | 5769 | GENERATED by work/build-source-lesson-map.mjs — do not hand-edit. |
| `structures.js` | 441 | Granular 3D targets: the structure sets a tap-to-identify item draws from, |
...
| `work/full-atlas-check.mjs` | Course-only replacement gate: removed surface plus retained packing algorithm. |  |
...
| `work/radiography-check.mjs` | The projection's physics, checked without a browser. |  |
