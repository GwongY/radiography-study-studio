...
| `outputs/assets/physiology/*.json` | **Generated** — the curated tube routes for a layer (`work/build-physiology-paths.mjs`), one file per layer, fetched with the layer it belongs to and cached beside the GLB. Never hand-edit; `outputs/physiology-paths.js` is the generated manifest that names them and stamps the cache key. |
...
| `physiology-path.js` | Pure rest-space tube geometry: welded graph, geodesic field, centreline fit, the discovery GATE that accepts or refuses a route by name, the travelling-constriction map and its Jacobian, and `PATH_SHAPE_GLSL` — the one copy of that map the shader uses for both position and normal. No three.js, no DOM, so `work/` runs it. |
...
node work/physiology-path-deform-check.mjs #   and for the peristalsis shader in
node work/physiology-path-model-check.mjs  #   live-physiology.js. The first two
