 ...
 */
import { $$, MEMORY_METHODS, describeSource, esc, priorSources, sourceMetaFor, sourceRoleLabel, sourceSetLabel } from './imports.js';
...
function sourceTableHTML(refs, heading, lessonId = '') {
  const rows = refs.map((r) => {
    const d = describeSource(r);
    const meta = sourceMetaFor(r.ref, lessonId);
    return `<tr>
      ...
      <td>${meta ? `${esc(sourceSetLabel(meta.set))} · ${esc(sourceRoleLabel(meta.role))}` : 'Source record not in current Y1S1 map'}</td>
    </tr>`;
  }).join('');
  return `${heading ? `<div class="subhead">${esc(heading)}</div>` : ''}<table class="srctable"><thead><tr><th>File</th><th>Subject</th><th>Folder</th><th>Page / section</th><th>Type</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
export function openSourceDialog(item, question) {
  ...
  $$('sourceBody').innerHTML = `
    ...
    ${question && question.src ? sourceTableHTML([question.src], 'Source for the answer you just saw', item.id) : ''}
    ${sourceTableHTML(item.sourceRefs, question && question.src ? 'Sources for the whole item' : '', item.id)}
