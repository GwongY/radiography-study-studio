...
export const HSS_TERMINOLOGY = [
  {
    ...
    skills: [
      'Palms forward is not a detail of the stance — it is the switch the whole map hangs on. Palms back and the radius crosses the ulna, so "lateral thumb" quietly becomes false: every forearm laterality question is answered by a rotation you make at the wrist.',
      'Anterior means the front of the body, not the front of the room. A prone patient\'s "anterior elbow" faces the couch, because the terms are pinned to the anatomical position and not to however the patient is lying — the mistake is re-anchoring the word to the scan position.',
    ],
  ...
  },
  {
    ...
    skills: [
      'The trap is hearing "Regional Anatomy of the Thorax" as revision of the two system lectures. It answers a different question: not "how does the cardiovascular system work" but "what sits against what inside one cavity" — the systemic lectures hand you the parts, the regional lecture hands you the neighbourhood.',
    ],
  ...
  },
  {
    ...
    skills: [
      'Proximal and distal are the pair that dies at the trunk: they measure distance from a limb\'s point of attachment, so between two trunk regions — sternum to scapula, say — the pair cannot be the answer no matter how plausible it sounds. No limb, no attachment point, no proximal.',
      'The thumb is lateral only because the palms face forward. Same hand, palms back, and the thumb reads medial — laterality below the elbow is a consequence of anatomical position, not a property of the hand itself.',
      'The most common error in this list is not ignorance but partner-swapping: knowing the term and producing its opposite. That is why they are learned as five opposed pairs — the partner is the answer half the time.',
    ],
  ...
  },
  {
    ...
    skills: [
      'Coronal and sagittal are both vertical cuts, so orientation cannot tell them apart — the split can. Coronal is the only plane that separates anterior from posterior; the sagittal family is the only one that separates left from right. Name the pair you want first, and the plane follows.',
      'Sagittal is not short for mid-sagittal. Any number of sagittal sections can be cut parallel to the midline, but only the median one divides the body into left and right halves — the "mid-" is doing the definitional work.',
      'Every plane carries a second name the glossary treats as equal: frontal = coronal, horizontal = transverse, median = mid-sagittal. The exam is free to use either, so an answer keyed to only one name is half an answer.',
    ],
  ...
  },
  {
    ...
    skills: [
      'Pericardial and peritoneal sound like the same word twice and live in different trunks: the pericardial cavity is inside the thorax around the heart, the peritoneal cavity inside the abdomen. The root after "peri-" carries the address — cardium (heart) versus peritoneum (abdominal lining).',
      'Four quadrants and nine regions are two grids over the same abdomen, not two names for one scheme. Right hypochondriac translates to right upper quadrant, but the quadrant answer throws away height: the quadrant cannot say whether the pain sits at rib-cartilage level or nearer the waist, and that loss is the whole trade-off between the schemes.',
    ],
